...
from passlib.hash import bcrypt as bcrypt_lib
...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://postgres:luRInCypjU9hBxWP@db.mbqxvjdrjuamhbnpsps.supabase.co:5432/postgres",
)
...
DEFAULT_SETTINGS = {
    ...
    "recharge_rate": "5",
    "new_user_credits": "10",
    "admin_password": bcrypt_lib.hash(ADMIN_PASSWORD),  # bcrypt hash 存储
...
async def register(req: RegisterRequest):
    with get_db() as conn:
        ...
        cur.execute("INSERT INTO users (username, email, password_hash, credits) VALUES (%s, %s, %s, %s) RETURNING id",
                     (req.username, req.email, bcrypt_lib.hash(req.password), nc))
...
async def login(req: LoginRequest):
    with get_db() as conn:
        ...
        user = cur.fetchone()
        if not user or not bcrypt_lib.verify(req.password, user["password_hash"]):
...
async def admin_login(req: LoginRequest):
    ...
    pwd_match = False
    try:
        pwd_match = bcrypt_lib.verify(req.password, stored_pwd)
    except Exception:
        # 不是 hash 格式，按明文比较
    ...
    if not stored_pwd.startswith("$2"):
        set_setting("admin_password", bcrypt_lib.hash(req.password))
...
async def admin_change_password(req: AdminPasswordRequest, admin: dict = Depends(get_admin_user)):
    ...
    pwd_match = False
    try:
        pwd_match = bcrypt_lib.verify(req.old_password, stored_pwd)
    except Exception:
    ...
    # 用 bcrypt hash 存储新密码
    hashed = bcrypt_lib.hash(req.new_password)
